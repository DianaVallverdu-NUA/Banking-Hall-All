...
class Particle {
  ...
  constructor() {
    ...
    let radius = Math.min(windowWidth, windowHeight) /  2;
  ...
  update() {
    ...
    var dir = createVector(windowWidth /  2, windowHeight /  2).sub(this.pos);
    ...
    // globalParticlePositions.push(this.pos.x); //store the x position for gradient map
  }
  ...
  calculateGradientColor() {
    ...
    let gradientPosition = gradientposIris
    return lerpColor(color('#04D9FF'), color('#B5FF00'), gradientPosition);
  }
  ...
  edges() {
    if (this.pos.x > windowWidth) { // Check if past the right edge
    ...
    if (this.pos.x <  0) { // Check if past the left edge
      this.pos.x = windowWidth; // Wrap around to the right edge
      this.updatePrev(); // Update the previous position
    }
    if (this.pos.y > windowHeight) { // Check if past the bottom edge
      this.pos.y =  0; // Wrap around to the top edge
      this.updatePrev(); // Update the previous position
    }
    if (this.pos.y <  0) { // Check if past the top edge
      this.pos.y = windowHeight; // Wrap around to the bottom edge
      this.updatePrev(); // Update the previous position
    }
  }
}